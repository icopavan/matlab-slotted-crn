...
%     
% 	%genie
%     total_reward(t) = sum(total_reward_random_record);
%     reward_genie(t) = sum(matrix_genie_sum(1:t));
%     regret(t) =  reward_genie(t)-sum(total_reward_random_record);
%     
%     [total_reward(t) reward_genie(t) regret(t)] 
% end
...
for t=1:T
    ...
    count_captured_channel(fixed_channel)=count_captured_channel(fixed_channel)+1;
	x = 1;
    if ismember(captured_index,target_index)
        total_reward_fixed = total_reward_fixed + x;  % 待改进 收益定为一个0到1的均匀分布
        total_reward_channel(fixed_channel)=total_reward_channel(fixed_channel)+x;
    end
    total_reward_fixed_record(t) = total_reward_fixed;
    
        
	%genie
    total_reward(t) = total_reward_fixed_record(t);
    reward_genie(t) = sum(matrix_genie_sum(1:t));
    regret(t) =  reward_genie(t)-total_reward(t);
