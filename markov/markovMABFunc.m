function [capture_rate,trm_su1,sniffer_channel_history,sniffer_observed_histroy,slot_capture] = markovMABFunc( trafficmat,num_ch,num_slot,genie_vector )
    ...
    if updtMode==1 % 看连续捕获 估测算法有问题 去了解一下HMM　Estimation算法
        if t>1 % 从t-1开始判断起
            if slot_capture(t-1)==1 && slot_capture(t)==1 % 连续捕获
               % update
                c_su1(sniffer_channel_history(t-1),sniffer_channel_history(t)) = c_su1(sniffer_channel_history(t-1),sniffer_channel_history(t)) + 1;
            end
            %update
            trm_su1 = count2prob(c_su1);
            msd_su1 = getMarkovStableDistribution( trm_su1,num_ch );
